...
class SortingOptions(BaseModel):
    ...
    @classmethod
    def from_request(cls, request: Request) -> SortingOptions | None:
        query_params: MultiDict = MultiDict(request.query_params)
        sort_by: str | None = query_params.pop('sortBy')
        order: int | None = query_params.pop('order')
        nearest_point_lat: float| None = query_params.pop('distanceFromLat')
        nearest_point_long: float| None = query_params.pop('distanceFromLong')
        max_distance: float | None = query_params.pop('maxDistance')
